 ...
 *
 * Initial subscription policy (no Trial status yet):
 * - planCode / planName: STARTER (catalog defaults)
 * - status: ACTIVE — platform intentionally onboarded the tenant; workshop access works
 *   under existing workshop-access rules (ACTIVE | PAST_DUE)
 * - paymentStatus: PENDING — not marked paid; verify/mark-paid is a separate platform step
 * - startsAt: now
 * - expiresAt / currentPeriodEnd: null until platform sets a paid term window
 * - termMonths: 12 (default catalog term length)
 * - limits: STARTER maxBranches / maxStaff from plan-catalog
 *
